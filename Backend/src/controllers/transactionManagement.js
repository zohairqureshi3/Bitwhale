...
exports.store = async (req, res) => {
    try {
        ...
        if (!getTransactionFee) {
            ...
            res.status(200).json({ success: true, message: "Transaction Fee set successfully", transactionManagement_ })
        } else {
...
exports.show = async function (req, res) {
    ...
    res.status(200).json({ success: true, message: "List of permissions associated with transactionFee", transactionManagement })
};
...
exports.update = async function (req, res) {
    try {
        ...
        if (transactionManagement) return res.status(200).json({ transactionManagement, message: 'Transaction Fee updated successfully' });
...
exports.getFee = async function (req, res) {
    const transactionFee = await TransactionManagement.findOne({ currencyId: req.body.currencyId }, { fee: 1 })
    if (transactionFee)
